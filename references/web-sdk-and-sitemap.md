...
| `clearPersistedIdentities()` | Clears known identities (e.g. `loyaltyId`, `email`) from SDK storage; fires `OnClearPersistedIdentities`. Documented on the Integrations page, not the Identity page |
...
- Beacon-injected on every translated event: `category`, `dateTime` (`yyyy-MM-dd'T'HH:mm:ss.SSS'Z'` only; used for partitioning), `deviceId`, `eventId`, `eventType` (schema `developerName`), `sessionId`. `interactionName` is required on catalog, cart, order and profile rows; the consentLog mapping does not list it. Custom `attributes.*` → `attributeCustomFieldN` and must be added to the schema manually [src](https://developer.salesforce.com/docs/data/salesforce-interactions-sdk/guide/c360a-api-translating-sdk-events-to-web-connector-schemas.html).
...
| Website Engagement (Dynamic Content, formerly Manual Content) | `userEngagement` | `personalization-view`, `personalization-click` (defaults) | no `id`/`type` | Website Engagement |
...
- **Profiles/server API**: MCP Unified Customer Profile and Event/Channel APIs; SP reads the Data 360 real-time profile data graph and servers call `/personalization/decisions` (§10).

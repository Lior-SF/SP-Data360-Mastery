...
- Field notes: every web event row bills Streaming Pipeline, including pages without experiences; each rotated device ID is a new source profile to unify; and match-rule, reconciliation, filter or new-field mapping changes trigger billable full ruleset reruns [src](https://help.salesforce.com/s/articleView?id=data.c360_a_billing_considerations_for_identity_resolution.htm&release=264.0.0&type=5), so batch them. Identity resolution limits and tactics: [field-guide-data.md](field-guide-data.md) §1, §5.

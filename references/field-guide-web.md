...
- **The browser identifier must equal the CRM value byte for byte.** A party identifier match is an exact match of Identification Number, Identification Name and Party Identification Type [src](https://help.salesforce.com/s/articleView?id=data.c360_a_identity_resolution_unify_partyidentifier.htm&release=264.0.0&type=5). A digest, a variant with or without a check digit, leading zeros, or case or whitespace drift never match, and nothing reports an error. **Do:** before UAT, read one real signed-in value in the console and find the same string in Party Identification from the CRM source (compare `LENGTH` and case). Normalize with `String(v ?? '').trim()` only. Don't case-fold or strip characters unless the CRM feed does the same.

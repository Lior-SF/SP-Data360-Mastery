...
- Why filter by point (inference): without it, a click on point B inside the window could be credited to an earlier view of point A. `Content Match` may do the same job; its behavior for manual content is UNVERIFIED.

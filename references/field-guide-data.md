...
- **Case sensitivity is opt-in.** Without `Case Sensitive`, uppercase and lowercase letters match [src](https://help.salesforce.com/s/articleView?id=data.c360_a_match_rules_advanced_settings.htm&release=264.0.0&type=5). **Do:** tick it for case-significant tokens (mixed-case federated or random IDs), batched with other rule changes (§1.3). Whether real-time runs honor it is UNVERIFIED.
...
- **Map web `deviceId` to both Party Identification ID and Party** [src](https://developer.salesforce.com/docs/marketing/einstein-personalization/guide/integrate-salesforce-interactions-sdk.html). With only one, rows collide or float unattached, and identity resolution matches nothing.
